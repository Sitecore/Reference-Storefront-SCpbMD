...
namespace Sitecore.Commerce.Storefront.Managers.Storefront
{
    ...
    public class WishListManager : BaseManager
    {
        ...
        /// <returns>The manager response with the wish list as the result.</returns>
        public virtual ManagerResponse<RemoveWishListLinesResult, WishList> RemoveWishListLines([NotNull] CommerceStorefront storefront, [NotNull] VisitorContext visitorContext, string wishListId, IEnumerable<string> lineIds)
        {
            Assert.ArgumentNotNull(storefront, "storefront");
            Assert.ArgumentNotNull(visitorContext, "visitorContext");
            Assert.ArgumentNotNull(lineIds, "lineIds");
            Assert.ArgumentNotNullOrEmpty(wishListId, "wishListId");
        ...
        /// </returns>
        public virtual ManagerResponse<UpdateWishListLinesResult, WishList> UpdateWishListLine([NotNull] CommerceStorefront storefront, [NotNull] VisitorContext visitorContext, [NotNull] string wishListId, [NotNull] string productId, [NotNull] string variantId, uint quantity)
        {
            Assert.ArgumentNotNull(storefront, "storefront");
            Assert.ArgumentNotNull(visitorContext, "visitorContext");
            Assert.ArgumentNotNullOrEmpty(wishListId, "wishListId");
            ...
            {
                Product = new CommerceCartProduct { ProductId = productId, ProductVariantId = variantId },
                Quantity = quantity
            };

            return this.UpdateWishListLines(storefront, visitorContext, wishListId, new List<WishListLine> { wishListLine });
        }
        ...
        public virtual ManagerResponse<AddLinesToWishListResult, WishList> AddLinesToWishList([NotNull] CommerceStorefront storefront, [NotNull] VisitorContext visitorContext, AddToWishListInputModel model)
        {
            ...
            {
                ProductCatalog = model.CatalogName,
        ...
        /// <returns>The manager response with the wish list as the result.</returns>
        public virtual ManagerResponse<UpdateWishListResult, WishList> UpdateWishList([NotNull] CommerceStorefront storefront, [NotNull] VisitorContext visitorContext, string wishListId, string wishListName, bool isFavorite)
        {
            Assert.ArgumentNotNull(storefront, "storefront");
            Assert.ArgumentNotNull(visitorContext, "visitorContext");
            Assert.ArgumentNotNullOrEmpty(wishListId, "wishListId");
            ...
            var request = new UpdateWishListRequest(new WishList { UserId = visitorContext.UserId, CustomerId = visitorContext.UserId, ExternalId = wishListId, Name = wishListName, IsFavorite = isFavorite, ShopName = storefront.ShopName });
        ...
        protected virtual void PopulateStockInformation(CommerceStorefront storefront, WishList wishList)
        {
            ...
            var stockInfos = this.InventoryManager.GetStockInformation(storefront, productList).Result;
            if (stockInfos == null)
            {
                return;
            }
            ...
            foreach (var stockInfo in stockInfos)
            {
                ...
                var item = wishList.Lines.Single(line => line.Product.ProductId.Equals(stockInfo.Product.ProductId, StringComparison.OrdinalIgnoreCase));
                item.Product.StockStatus = new Entities.Inventory.StockStatus(System.Convert.ToInt32((decimal)stockInfo.Properties["OnHandQuantity"]), stockInfo.Status.Name);
