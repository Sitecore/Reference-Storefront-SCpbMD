...
namespace Sitecore.Commerce.Storefront.SitecorePipelines
{
    ...
    public class TriggerVisitedProductDetailsPagePageEvent : TriggerPageEvent
    {
        ...
        protected override Dictionary<string, object> GetPageEventData(Pipelines.ServicePipelineArgs args)
        {
            ...
            if (request != null)
            {
                data.Add(Constants.PageEventDataNames.ProductId, request.ProductId);
                data.Add(Constants.PageEventDataNames.ParentCategoryName, request.ParentCategoryName ?? string.Empty);
